...
describe('6-CCB-UI Notification Draft Templates', () => {
  beforeEach( function()  {
  ...
  it('Draft Template Name Max Length Validation', function(){
    ...
    cy.get('#draftName').invoke('attr', 'maxlength').should('contain', pageElementsObj.inputMaxLength);
  });
 ...
 it('Push Draft Mandatory Check all fields', function(){
  ...
  it('Letter Draft Mandatory Check all fields', function(){
    cy.LoginApp(this.userCred.email,this.userCred.password);
    pageElementsObj.testDraftSelectLanag(this.commonVars);
    pageElementsObj.testDraftSelectChannel('Letter','Letter-Draft');
    cy.get('.textEditor').find('iframe.k-content').should('exist');
    cy.get('#saveDraftBtn').click();
    cy.get('#programAlert').find('div.ng-binding').contains('Please enter');
  });
  it('Letter Draft Template Name Mandatory Validation', function(){
    cy.LoginApp(this.userCred.email,this.userCred.password);
    pageElementsObj.testDraftSelectLanag(this.commonVars);
    pageElementsObj.testDraftSelectChannel('Letter','Letter-Draft');
    cy.get('.textEditor').find('iframe.k-content').should('exist');
  ...
  it('Letter Draft Body Mandatory Validation', function(){
    cy.LoginApp(this.userCred.email,this.userCred.password);
    pageElementsObj.testDraftSelectLanag(this.commonVars);
    pageElementsObj.testDraftSelectChannel('Letter','Letter-Draft');
    cy.get('#draftName').type(this.commonVars.draftTempName,{force:true},{delay:10}).should('have.value',this.commonVars.draftTempName);
